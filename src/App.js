...
function App() {
  ...
  return (
    <div className="App">
      <div className="englobante d-flex  jusify-content-center w-50 mx-auto">
        <CardPresentation active={active} verifActive={verifActive} />
        <div className="d-flex flex-wrap">
      
          {/* Le daily */}
          {active == 1 && data.map((item, index) => {
              
              return (
                <Card key={index}
                  class1={item.title.toLowerCase()}
                  txt={item.title}
                  hrs={item.timeframes.daily.current + "hrs"}
                  txt2={"Last Week-" + item.timeframes.daily.previous + "hrs"}
                  svgName={tabSvg[index]} />
              )
            }
          )}
          ...
          {active == 2 && data.map((item, index) => {
            return (
              <Card key={index}
                class1={item.title.toLowerCase()}
                txt={item.title}
                hrs={item.timeframes.weekly.current + "hrs"}
                txt2={"Last Week-" + item.timeframes.weekly.previous + "hrs"}
                svgName={tabSvg[index]} />
              )
            }
          )}
          ...
          {/* le monthly */}
          {active == 3 && data.map((item, index) => {
            return (
              <Card key={index}
                class1={item.title.toLowerCase()}
                txt={item.title}
                hrs={item.timeframes.monthly.current + "hrs"}
                txt2={"Last Week-" + item.timeframes.monthly.previous + "hrs"}
                svgName={tabSvg[index]} />
              )
            }
          )}
